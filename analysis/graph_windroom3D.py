...
def generate_3d_static_view():
    ...
    for idx, current_N in enumerate(N_values):
        ...
        #ax.set_title(f"Sistema Adattivo 3D | N = {current_N}", fontweight='bold', fontsize=12)
        ...
        p.L = 3.0
        p.theta_ref = 30.0
        p.payload_shape = 'cylinder'  # Opzioni: 'box', 'cylinder', 'sphere'
        p.pay_l = 1.8            # Lunghezza (asse X)
        p.pay_w = 0.8            # Larghezza (asse Y)
        p.pay_h = 0.2            # Altezza (asse Z)
        p.R_disk = 0.6           # Raggio (solo per cylinder o sphere)
        ...
        # CoM Marker
        #ax.plot([real_com_pos[0]], [real_com_pos[1]], [real_com_pos[2]], 'ko', ms=3, zorder=200, label='Payload CoM')
        ...
        if p_adapt.N >= 3:
            ...
            uav_center[2] += 0.5 
            vec_A, vec_B = np.zeros(3), np.zeros(3)
            for i in range(p_adapt.N):
                angle_i = 2 * np.pi * i / p_adapt.N
                diff = curr_uavs[:, i] - uav_center
            ...
            ring_vals = (uav_center[:, np.newaxis] + 
                         vec_A[:, np.newaxis] * np.cos(theta_loop) + 
                         vec_B[:, np.newaxis] * np.sin(theta_loop))

        if w_mag > 0.1:
            w_dir = v_wind_vec / w_mag
            w_start = pay_hover_pos + np.array([0, 0, 2.5]) 
            ax.quiver(w_start[0], w_start[1], w_start[2], 
                      w_dir[0], w_dir[1], w_dir[2], 
                      length=1.5 + (w_mag * 0.1), color='deepskyblue', 
                      arrow_length_ratio=0.3, linewidth=3, alpha=0.8)

        m_tot = p_adapt.m_payload + p_adapt.m_liquid
        F_tot_vec = np.array([0, 0, -m_tot * p_adapt.g]) + F_wind_actual
        ...
        # 2. RISOLUZIONE COMPRESSIONE 3D (Scala 1:1:1 REALE)
        ax.set_box_aspect((
            x_range[1] - x_range[0], 
            y_range[1] - y_range[0], 
            z_range[1] - z_range[0]
        ...
        ax.set_axis_off()  # <-- RIMUOVE TUTTE LE MISURE E LE SCATOLE DEGLI ASSI
