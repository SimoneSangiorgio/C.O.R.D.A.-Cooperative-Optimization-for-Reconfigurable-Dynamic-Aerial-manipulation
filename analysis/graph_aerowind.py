...
# ---------------------------------------------------------
# NUOVA FUNZIONE COMPARATIVA VENTO CRESCENTE (PIANO XZ)
# ---------------------------------------------------------

def generate_wind_comparison_xz():
    p_base = SysParams()
    ...
    # Valori di vento crescenti
    wind_vectors = [
        np.array([0.0, 0.0, 0.0]),  # 1° grafico: Zero vento
        np.array([3.0, 0, 0.0]),  # 2° grafico: Vento
        np.array([4.5, 0, 0.0])   # 3° grafico: Vento più forte
    ]
    ...
    for i, v_wind_vec in enumerate(wind_vectors):
        ...
        p_rigid.lambda_aero = 1.0
        p_rigid.lambda_static = 1.0
        ...
        # Puoi scommentare questa riga per mostrare un titolo su ciascun plot
        # ax_xz.set_title(f"Vento = {wind_mag:.1f} m/s", fontsize=14, fontweight='bold')

    plt.tight_layout()
    plt.show()

if __name__ == "__main__":
    generate_wind_comparison_xz()