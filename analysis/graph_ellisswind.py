...
def plot_xy_view(ax, p_base, pos_ref, pos_adapt, att_vecs_rigid, att_vecs_adapt, 
    ...
    if np.linalg.norm(w_vec_2d) > 0.1:
        ...
        ax.arrow(start_pt[0]-0.5, start_pt[1]-0.5, direction[0], direction[1],
                 width=limit*0.025 if limit else 0.1, color="#0092BE", alpha=0.6, ec='None', zorder=0)
    
        wind_speed = np.linalg.norm(v_wind_vec)
        ax.text(start_pt[0]*1.06-0.5, start_pt[1]*1.06-0.5, 
                f"{wind_speed:.1f} m/s", 
    ...
    if M_ext is not None and abs(M_ext[2]) > 0.1:
        ...
        arc = FancyArrowPatch(
            posA=(com_2d[0] + r_arc, com_2d[1]), 
            posB=(com_2d[0], com_2d[1] + sign * r_arc),
            connectionstyle=f"arc3,rad={sign * 0.5}",
            arrowstyle="Simple,head_width=5,head_length=8",
            color='#0092BE', lw=3, zorder=0, label="Torque Z")
        ax.add_patch(arc)
        ax.text(com_2d[0] + r_arc, com_2d[1] + sign*r_arc, f" {M_ext[2]:.1f}Nm", color='#0092BE', fontweight='bold')
...
# ---------------------------------------------------------
# NUOVA FUNZIONE COMPARATIVA VENTO CRESCENTE (PIANO XY)
# ---------------------------------------------------------

def generate_wind_comparison_xy():
    p_base = SysParams()
    ...
    # 3 valori di vento crescenti
    wind_vectors = [
        np.array([0.0, 0.0, 0.0]),  # 1° grafico: Zero vento
        np.array([6.5, 6.5, 0.0]),  # 2° grafico: Vento a 3 m/s tutto sull'asse Y
        np.array([8.5, 8.5, 0.0])   # 3° grafico: Vento diagonale (es. 3 m/s su X e 3 m/s su Y)
    ]
    ...
    for i, v_wind_vec in enumerate(wind_vectors):
        ax_xy = axes[i]
        
        # Calcoliamo la magnitudine (modulo) partendo dal vettore per i calcoli successivi
        ...
        # 1. Calcolo Formazione Rigida (riferimento grigio)
        ...
        # 2. Calcolo Formazione Adattiva (con lambda = 1)
        ...
        # Plot su piano XY per il dato asse e aggiornamento del titolo
        ...
        #ax_xy.set_title(f"Vento = {wind_mag} m/s", fontsize=14, fontweight='bold')

    plt.tight_layout()
    plt.show()

if __name__ == "__main__":
    # Eseguiamo la nuova configurazione con subplots 1x3
    generate_wind_comparison_xy()